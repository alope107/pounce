...
inline bn::fixed_point clamp_point(int min_x, int min_y, int max_x, int max_y, const bn::fixed_point& point) {
    return {
...
inline bn::fixed_point clamp_point(bn::rect bounds, const bn::fixed_point& point) {
    return clamp_point(bounds.left(), bounds.top(), bounds.right(), bounds.bottom(), point);
}
